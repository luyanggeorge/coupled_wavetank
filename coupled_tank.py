...
input_data, test_case, FWF, margin = test_case()
...
while t<=Tend+smallfac:
    ...
    if t_save-smallfac < t:
        ...
        zero_index = np.where(h_fv.dat.data < tolerance)[0][0]
        x_w = sw_mesh.coordinates.dat.data[zero_index]

        #__________________________ Output h and hu at x=x_c ___________________________#
        h_xc = h_fv.dat.data[0]
        u_xc = hu_fv.dat.data[0]/h_xc

        with open(energy_file, 'a') as e_f:
            e_f.write(f"{t:8.4f} {E_dw:.6e} {E_sw:.6e} {x_w:10.6f} {h_xc:10.6f} {u_xc:10.6f}\n")
